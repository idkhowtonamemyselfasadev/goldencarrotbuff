...
`require-resource-pack=true` matters: a client without the pack would see
purple-and-black cubes for every food. With it, a player who declines is sent
back to the server list instead. The SHA-1 has to match the zip exactly; it is
in `release/goldencarrotbuff-pack.zip.sha1` and printed by `pack.py`. Whenever
the pack changes, publish the new zip and update both lines.

If you would rather not force a pack, set `CUSTOM_MODELS = False` in `foods.py`
and regenerate: the foods then borrow vanilla item textures and need no pack.
...
`item_name`, `item_model` (the pack model, or any vanilla texture), `lore`,
`rarity`, `food`, `consumable` with its effects, and a `custom_data` tag naming
the food. Bread is the base because nothing in vanilla crafts, smelts or brews
with bread, so a custom food can never sneak into another recipe. The custom
foods do not stack with plain bread.
...
4. Add the resource pack lines to `server.properties` (see below)
5. Start the server
