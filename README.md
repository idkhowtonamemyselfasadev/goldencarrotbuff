...
### The resource pack (custom 3D models)

Every food has its own 3D model and texture, generated by `pack.py` in
Minecraft's model format (the same JSON Blockbench and Blender export). They
live in a resource pack the **server hands to every player on join**, so nobody
installs anything by hand. A preview of all 100 is in `release/preview.png`.

The pack is published on GitHub. Put these lines in the server's
`server.properties` and restart:

```
resource-pack=https://github.com/idkhowtonamemyselfasadev/goldencarrotbuff/releases/download/v1.0.0/goldencarrotbuff-pack.zip
resource-pack-sha1=d7bf454a616da829eeded456929ae7d590d1914b
...
four colours in `pack.py`; run `python3 pack.py` to rebuild the pack zip and
...
https://github.com/idkhowtonamemyselfasadev/goldencarrotbuff/releases/tag/v1.0.0
